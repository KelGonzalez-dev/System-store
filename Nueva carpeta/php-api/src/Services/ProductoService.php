...
class ProductoService
{
    ...
    {
        $stmt = $this->db->prepare(
            'INSERT INTO productos (codigo, nombre, descripcion, descripcion_larga, precio, imagen_url)
             VALUES (?, ?, ?, ?, ?, ?)'
        );
        ...
        if (empty($req['codigo'])) {
            $generated = sprintf('GB-%04d', $id);
            $upd = $this->db->prepare('UPDATE productos SET codigo = ? WHERE id = ?');
            $upd->execute([$generated, $id]);
        }
